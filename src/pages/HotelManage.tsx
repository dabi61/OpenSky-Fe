...
const HotelManage: React.FC = () => {
  const { hotelList, loading, getHotelBySatus } = useHotel();
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [page, setPage] = useQueryState("page", "1" as string);
  const [totalPages, setTotalPages] = useState(0);
  const [selectedStatus, setSelectedStatus] = useQueryState<HotelStatus>(
    "status",
    HotelStatus.ACTIVE
  );
  ...
  useEffect(() => {
    const fetchHotel = async () => {
      try {
        const data = await getHotelBySatus(selectedStatus, parseInt(page), 1);
