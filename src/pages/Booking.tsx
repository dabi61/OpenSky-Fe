...
const Booking: React.FC = () => {
  ...
  const bookingRoomList: RoomType[] = JSON.parse(
    localStorage.getItem("bookingRoomList") || "[]"
  );
  const bookingTourList: TourType[] = JSON.parse(
    localStorage.getItem("bookingTourList") || "[]"
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border-t pt-3 flex justify-between text-lg font-semibold">
                      ...
                      <span className="text-blue-600">
                        {(() => {
                          if (bookingRoomList && bookingRoomList.length > 0) {
                            return (
                              Intl.NumberFormat("vi-VN").format(
                                bookingRoomList.reduce(
                                  (sum, room) =>
                                    sum + room.price * room.maxPeople,
                                  0
                                )
                              ) + " VNĐ"
                            );
                            ...
                            bookingTour &&
                            Object.keys(bookingTour).length > 0
                          ) {
                            return (
                              Intl.NumberFormat("vi-VN").format(
                                bookingTour.price
                ...
                {bookingRoomList && bookingRoomList.length > 0 && (
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Khách sạn:</span>
                      <span className="font-medium">
                        {bookingRoomList[0].hotelName}
                      </span>
                    </div>

                    <div className="space-y-4">
                      {bookingRoomList.map((room, index) => (
                        <div
                          key={index}
                    ...
                    <div className="border-t pt-3 flex justify-between text-lg font-semibold">
                      ...
                      <span className="text-blue-600">
                        {Intl.NumberFormat("vi-VN").format(
                          bookingRoomList.reduce(
                            (sum, room) => sum + room.price * room.maxPeople,
                            0
                          )
                        )}
                        VNĐ
